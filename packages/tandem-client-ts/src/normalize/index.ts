...
export const idNormalizer = z.string().or(z.object({
    ...
    return val.id || val.runID || val.runId || val.run_id ||
        val.sessionID || val.sessionId || val.session_id ||
        val.missionID || val.missionId || val.mission_id ||
        val.instanceID || val.instanceId || val.instance_id;
});

// JSON fallback
export const jsonFallback = z.any().transform(val => val as Public.JsonValue);
export const jsonObjectFallback = z.record(z.string(), z.any()).transform(val => val as Public.JsonObject);
...
}).passthrough().transform((val): Public.SessionRecord => ({
    ...val,
...
export const SessionRunStateResponseSchema = z.object({
    active: z.object({
        runID: z.string().optional(),
        runId: z.string().optional(),
        run_id: z.string().optional(),
...
}).passthrough().transform((val): Public.SessionRunStateResponse => {
...
}).passthrough().transform((val): Public.RunNowResponse => ({
    ...val,
...
}).passthrough().transform((val): Public.RunRecord => ({
    ...val,
    runId: val.runId || val.runID || val.run_id,
...
}).passthrough().transform((val): Public.ResourceRecord => ({
    ...val,
...
export const MemoryItemSchema = z.object({
    ...
    run_id: z.string().optional(),
    runID: z.string().optional(),
}).passthrough().transform((val): Public.MemoryItem => ({
    ...val,
...
}).passthrough().transform((val): Public.EngineEvent => {
    return {
        ...val,
...
export function parseResponse<T>(schema: z.ZodType<T, any, any>, rawData: unknown, endpoint: string, status: number): T {
