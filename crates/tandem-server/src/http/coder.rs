...
pub(super) async fn coder_run_execute_next(
    ...
    if !matches!(
        record.workflow_mode,
        CoderWorkflowMode::IssueTriage | CoderWorkflowMode::IssueFix | CoderWorkflowMode::PrReview
    ) {
        return Ok(Json(json!({
            "ok": false,
            "error": "execute_next is only wired for issue_triage, issue_fix, and pr_review right now",
