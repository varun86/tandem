...
function typeColor(t: string): string {
  return (
    TYPE_COLOR[t] ||
    (t.includes("error") ? "text-rose-400" : t.includes("tool") ? "text-sky-400" : "text-gray-300")
  );
}
...
function EventRow({ ev }: { ev: LiveEvent }) {
  ...
  const summary = (() => {
    ...
    if (pay.type === "approval.resolved")
      return `${String(props?.reply || "?")} → ${String(props?.tool || "?")}`;
    return "";
  })();
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`p-1.5 rounded-lg hover:bg-gray-800 transition-colors ${autoscroll ? "text-emerald-400" : "text-gray-600"}`}
              title="Auto-scroll"
            >
              <RefreshCw size={14} />
            </button>
