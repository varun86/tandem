...
- **Workflow learning API, state, runtime, candidate-generation, and planner-revision coverage**: Added focused HTTP, state, and runtime coverage for workflow learning candidate listing, review, promotion, revision-spawn guardrails, candidate dedupe, status transitions, approved-learning prompt injection, run-summary persistence, terminal-run candidate generation thresholds, and planner revision session creation so the new surfaces keep deterministic behavior as the feature hardens.
