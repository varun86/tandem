...
- **Bug Monitor triage now runs on Automation V2**: triage is no longer an orphaned context run that can sit forever in `Planning`. It now launches an Automation V2 run with inspect, research, validation, and fix-proposal nodes, and mirrors completed node outputs back into Bug Monitor artifacts for issue drafting.
