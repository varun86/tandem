...
Bug Monitor triage also now passes the resolved `workspace_root` into every Automation V2 triage node as explicit node input. Research guidance tells the agent to search inside that repo root and prefer calling `read` on at least one concrete source file before finalizing, but source-read evidence is now advisory for Bug Monitor's own artifacts instead of a hard publication gate. If a triage node can only preserve search evidence, tool limitations, uncertainty, and the original workflow failure, Bug Monitor still synthesizes and publishes the fallback issue instead of recursively failing on `no_concrete_reads`.

Blocked Automation V2-backed Bug Monitor triage runs are now treated as terminal enough for fallback summary synthesis. This means a failed triage workflow can still generate a GitHub issue that points at the original workflow failure, including the failing node id, timeout budget, repo/workspace context, and available tool evidence.
